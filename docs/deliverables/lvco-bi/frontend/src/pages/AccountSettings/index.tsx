...
type TabKey = 'profile' | 'password' | 'ai' | 'preferences'
...
const PREFS_KEY = 'lvco_preferences'
...
export default function AccountSettingsPage() {
  ...
  const handlePrefsSave = () => {
    try {
      ...
      setPrefsMsg({ type: 'success', text: '偏好设置已保存' })
    } catch {
      setPrefsMsg({ type: 'error', text: '保存失败：浏览器存储不可用' })
    }
  }
  ...
  return (
    <div className="flex-1 p-6 space-y-5">
      ...
      <div className="border-b border-border-light flex gap-0 -mb-px">
        ...
        <TabButton active={tab === 'preferences'} onClick={() => setTab('preferences')} icon={Palette} label="偏好设置" />
      </div>
      ...
      {tab === 'preferences' && (
        <div className="bg-white rounded-md shadow-card border border-border p-6">
          <h2 className="text-lg font-medium mb-1">偏好设置</h2>
          <p className="text-[12px] text-muted-foreground mb-4">个性化展示与本地化选项</p>
          <div className="space-y-4 max-w-md">
            <Field label="主题">
              <select
                value={prefs.theme}
              ...
              >
                ...
                <option value="dark">深色（实验性）</option>
            ...
            <Field label="语言">
              <div className="relative">
                ...
                <select
                  ...
                  onChange={e => setPrefs({ ...prefs, language: e.target.value as Preferences['language'] })}
            ...
            <Field label="日期格式">
              <select
                ...
                className="w-full px-3 py-2 border border-border rounded-md bg-input text-sm"
              >
                <option value="YYYY-MM-DD">2026-07-26（ISO）</option>
                <option value="DD/MM/YYYY">26/07/2026（欧式）</option>
                <option value="MM/DD/YYYY">07/26/2026（美式）</option>
              </select>
            </Field>
            <Field label="数字格式">
              <select
                value={prefs.numberFormat}
                onChange={e => setPrefs({ ...prefs, numberFormat: e.target.value as Preferences['numberFormat'] })}
                className="w-full px-3 py-2 border border-border rounded-md bg-input text-sm"
              >
                <option value="1,000.00">1,000.00（英式）</option>
                <option value="1.000,00">1.000,00（欧式）</option>
                <option value="1 000,00">1 000,00（法语）</option>
              </select>
            </Field>
            <StatusMsg msg={prefsMsg} />
            <SaveButton loading={false} onClick={handlePrefsSave} label="保存偏好" />
          </div>
        </div>
      )}
    </div>
...
function ToggleRow({
  ...
  return (
    <label className="flex items-start gap-3 px-3 py-2 border border-border rounded-md cursor-pointer hover:bg-muted/50">
      <button
        type="button"
