...
export async function describeStatistics(datasourceId: string) {
  const res = await apiClient.post('/statistics/describe', { datasource_id: datasourceId })
...
export async function correlationMatrix(datasourceId: string, fields?: string[]) {
  const res = await apiClient.post('/statistics/correlation', { datasource_id: datasourceId, fields })
...
export async function getRanking(datasourceId: string, params: RankingParams) {
  const res = await apiClient.post('/statistics/ranking', { datasource_id: datasourceId, ...params })
...
export async function getComparison(datasourceId: string, params: ComparisonParams) {
  const res = await apiClient.post('/statistics/comparison', { datasource_id: datasourceId, ...params })
...
export async function getSummary(datasourceId: string) {
  const res = await apiClient.post('/statistics/summary', { datasource_id: datasourceId })
