...
def _html_to_pdf_playwright(html: str) -> bytes:
    """Render HTML to PDF using Playwright's Chromium engine (sync, for thread pool)."""
...
async def _html_to_pdf_async(html: str) -> bytes:
    """Render HTML to PDF using Playwright's async API."""
...
.block-divider { border: none; border-top: 1px solid #e5e5e5; margin: 16px 0; }
...
async def export_canvas_pdf(
    ...
    for block in raw_blocks:
        ...
        if btype == "chart":
            ...
            chart_type = block_copy.get("chartType") or block_copy.get("renderer") or "bar"
            chart_images = []
    ...
    # Use Playwright async API to render HTML to PDF (avoids to_thread issues on Windows)
    pdf_bytes = None
    try:
        logger.info("export_pdf invoking async_playwright, html_len=%d", len(html_content))
        pdf_bytes = await _html_to_pdf_async(html_content)
    ...
    return Response(
        ...
        headers={"Content-Disposition": f'attachment; filename="canvas-{canvas.id}.pdf"'},
    )
