...
async def create_rule(
    ...
    # 注册到调度器（非阻塞，失败仅 log）
    await _safe_reload_scheduler(rule.id)
    return SuccessResponse(
...
async def update_rule(
    ...
    await db.commit()
    await db.refresh(rule)
    # 更新调度（enabled/schedule_time 变化都需 reload）
    await _safe_reload_scheduler(rule.id)
    return SuccessResponse(
        data=InsightRuleResponse.model_validate(rule).model_dump(mode="json", by_alias=True)
    )
...
async def delete_rule(
    ...
    await db.commit()
    # 从调度器移除
    await _safe_remove_scheduler(rule_id)
...
async def run_rule_now(
    ...
    try:
        await runner.run(db, rule, period_start, period_end)
    except Exception as e:
        import structlog
        structlog.get_logger("insights_api").warning("run_now_failed", error=str(e))
    await db.refresh(record)
    return SuccessResponse(
        data=InsightRecordRunResponse(
            record_id=record.id, status=record.status.value if hasattr(record.status, "value") else str(record.status)
        ).model_dump(mode="json", by_alias=True)
    )
...
async def list_records(
    ...
    """执行记录列表（可选 rule_id 过滤）"""
    q = select(InsightRecord, InsightRule.name).join(
        InsightRule, InsightRule.id == InsightRecord.rule_id
    ).where(InsightRecord.user_id == current_user.id)
    if rule_id is not None:
        q = q.where(InsightRecord.rule_id == rule_id)

    total = (await db.execute(
        select(func.count()).select_from(q.subquery())
    )).scalar() or 0

    q = q.order_by(InsightRecord.run_at.desc()).offset((page - 1) * page_size).limit(page_size)
    rows = (await db.execute(q)).all()

    items: list[InsightRecordSummary] = []
    for r, rule_name in rows:
        anomalies = r.detected_anomalies or {}
        items.append(InsightRecordSummary(
            id=r.id,
            rule_id=r.rule_id,
            rule_name=rule_name,
            run_at=r.run_at,
            ...
            has_anomalies=bool(anomalies.get("items")) if isinstance(anomalies, dict) else False,
            anomaly_count=len(anomalies.get("items", [])) if isinstance(anomalies, dict) else 0,
            report_id=r.report_id,
        ))

    pages = math.ceil(total / page_size) if total > 0 else 0
    return SuccessResponse(
        data=InsightRecordListResponse(
            items=items,
            total=total,
            page=page,
            page_size=page_size,
...
async def get_record(
    ...
    result = await db.execute(
        select(InsightRecord, InsightRule.name)
        .join(InsightRule, InsightRule.id == InsightRecord.rule_id)
        .where(InsightRecord.id == record_id, InsightRecord.user_id == current_user.id)
    )
    row = result.first()
    if row is None:
        raise HTTPException(
    ...
    detail = InsightRecordDetail(
        ...
        detected_anomalies=r.detected_anomalies or [],
        llm_tokens_input=r.llm_tokens_input,
        llm_tokens_output=r.llm_tokens_output,
    )
    return SuccessResponse(data=detail.model_dump(mode="json", by_alias=True))
...
async def discover_datasource(
    ...
    try:
        ...
        import structlog
        structlog.get_logger("insights_api").error("discover_failed", error=str(e))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"code": "DISCOVER_FAILED", "message": f"数据源扫描失败: {str(e)}"},
        )
    return SuccessResponse(
        data={
...
async def _safe_reload_scheduler(rule_id: uuid.UUID) -> None:
    """安全地重载调度（异常仅 log，不影响主流程）"""
    try:
        await insight_scheduler.reload_rule(rule_id)
    except Exception as e:
        import structlog
        structlog.get_logger("insights_api").warning(
            "scheduler_reload_failed", rule_id=str(rule_id), error=str(e)
        )


async def _safe_remove_scheduler(rule_id: uuid.UUID) -> None:
    """安全地从调度器移除规则（异常仅 log）"""
    try:
        await insight_scheduler.remove_rule(rule_id)
    except Exception as e:
        import structlog
        structlog.get_logger("insights_api").warning(
            "scheduler_remove_failed", rule_id=str(rule_id), error=str(e)
        )
