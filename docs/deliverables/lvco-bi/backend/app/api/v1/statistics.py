...
def _ensure_external_attached(ds: DataSource, schema_name: str) -> None:
    ...
    失败时抛 HTTPException(500),错误信息脱敏后返回。
    """
...
async def describe_statistics(
    ...
    if len(measure_fields) < 2:
        try:
            ...
            info_rows = await asyncio.to_thread(
                ...
                f'SELECT column_name, data_type FROM {table_ref_db.split(".")[0]}.information_schema.columns '
                f"WHERE table_name = ?",
                [table_ref_db.split('"')[-2] if '"' in table_ref_db else "data"],
            )
            if info_rows:
                extra_measures = []
    ...
    if not measure_fields:
        return SuccessResponse(data={"fields": [], "statistics": []})
    ...
    for field_info in measure_fields:
        ...
        sql = (
            f'SELECT '
            f'COUNT("{field_name}") as count, '
            f'AVG("{field_name}") as mean, '
            f'STDDEV("{field_name}") as std, '
            f'MIN("{field_name}") as min, '
            f'PERCENTILE_CONT(0.25) WITHIN GROUP (ORDER BY "{field_name}") as p25, '
            f'PERCENTILE_CONT(0.50) WITHIN GROUP (ORDER BY "{field_name}") as p50, '
            f'PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY "{field_name}") as p75, '
            f'MAX("{field_name}") as max, '
            f'SUM(CASE WHEN "{field_name}" IS NULL THEN 1 ELSE 0 END) as null_count '
            f'FROM {table_ref}'
        )
        try:
            result_row = await asyncio.to_thread(lambda: duckdb_client.execute(sql).fetchone())
        except Exception as e:
            raise HTTPException(
                status_code=500,
                detail={"code": "SQL_FAILED", "message": f"统计字段 {field_name} 失败: {str(e)[:200]}"},
            ) from e

        if result_row:
            count_val = result_row[0] or 0
            null_count_val = result_row[8] or 0
            stats.append({
                "field": field_name,
                "count": count_val,
                "mean": round(result_row[1], 2) if result_row[1] is not None else None,
                "std": round(result_row[2], 2) if result_row[2] is not None else None,
                "min": round(result_row[3], 2) if result_row[3] is not None else None,
                "p25": round(result_row[4], 2) if result_row[4] is not None else None,
                "p50": round(result_row[5], 2) if result_row[5] is not None else None,
                "p75": round(result_row[6], 2) if result_row[6] is not None else None,
                "max": round(result_row[7], 2) if result_row[7] is not None else None,
                "null_count": null_count_val,
                "null_rate": round(null_count_val / count_val, 4) if count_val > 0 else 1.0,
            })

    return SuccessResponse(data={"fields": [f["name"] for f in measure_fields], "statistics": stats})
...
async def correlation_analysis(
    ...
    for i in range(n):
        ...
        for j in range(i):
            try:
                sql = f'SELECT CORR("{selected[i]}", "{selected[j]}") FROM {table_ref}'
                row = await asyncio.to_thread(lambda: duckdb_client.execute(sql).fetchone())
    ...
    return SuccessResponse(data={"fields": selected, "matrix": matrix})
