...
def render_blocks_to_html(blocks: list[dict[str, Any]] | None) -> str:
    ...
    for block in blocks:
        ...
        if btype in ("title", "h1"):
            ...
            chart_imgs = block.get("_chart_images") or ([block["_chart_image"]] if block.get("_chart_image") else [])
            if chart_imgs:
                imgs_html = "".join(
                    f'<img src="{_escape(img)}" style="max-width:100%; height:auto; border-radius:8px; box-shadow:0 2px 8px rgba(0,0,0,0.08); margin-bottom:8px;" />'
                    for img in chart_imgs
                )
                parts.append(f'<div class="block block-chart">{imgs_html}</div>')
            else:
                data = block.get("data", {})
