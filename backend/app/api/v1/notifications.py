...
async def push_notification(
    ...
    """前端直接推送通知（数据源就绪、分析完成等）"""
    from app.models.notification import NotificationType
    type_ = body.type if body.type in (e.value for e in NotificationType) else "system"
    service = NotificationService(SQLAlchemyNotificationRepository(db))
    notif = await service.push(
        current_user.id,
        type_,
        title=body.title,
        body=body.body,
    ...
    return SuccessResponse(data=_serialize(notif))
