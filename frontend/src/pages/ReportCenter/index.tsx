...
export default function ReportCenter() {
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0">
      ...
      <div className="flex-1 p-6 overflow-auto">
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <div className="relative">
            ...
            >
              {statusFilter === 'draft' ? '草稿' : statusFilter === 'published' ? '已发布' : statusFilter === 'shared' ? '已分享' : '全部状态'}
              <ChevronDown className="w-3.5 h-3.5 text-muted-foreground" />
            </button>
            {showStatusDropdown ? (
              <div className="absolute top-full left-0 mt-1 w-32 bg-white border border-border rounded-md shadow-lg z-50">
                {[{v:null,l:'全部状态'},{v:'draft',l:'草稿'},{v:'published',l:'已发布'},{v:'shared',l:'已分享'}].map(o => (
      ...
      {/* 分享弹窗 */}
      <ModalShell
        open={Boolean(shareModalReport)}
        onClose={closeShareModal}
        title={`分享报表${shareModalReport ? `：${shareModalReport.title}` : ""}`}
      >
        <div className="space-y-4 text-[13px]">
          {shareLoading ? (
            ...
            <>
              <p className="text-card-foreground">
                将此链接分享给他人，对方无需登录即可查看报表内容（链接有效期 30 天）。
              </p>
              <div className="flex items-center gap-2">
                <input
                  ...
                  readOnly
                  value={shareUrl}
                  className="flex-1 px-3 py-2 text-[13px] rounded-md border border-border bg-muted text-card-foreground focus:outline-none"
                />
                <button
                  onClick={copyShareUrl}
                  className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-[13px] font-medium text-white bg-primary hover:bg-primary-hover transition-colors flex-shrink-0"
                >
                  {shareCopied ? (
                    <CheckCircle2 className="w-4 h-4" />
                  ) : (
