...
export async function updateSession(sessionId: string, title: string): Promise<AISession> {
  const response = await apiClient.patch(`/ai/sessions/${sessionId}`, { title });
  return unwrapApi<AISession>(response.data);
}
...
export async function applyClean(payload: AICleanRequest): Promise<{
  results: Array<{ field: string; action: string; success: boolean; affected_count: number; error?: string }>;
}> {
  const response = await apiClient.post("/ai/clean/apply", payload);
